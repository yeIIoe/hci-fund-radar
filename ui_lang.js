...
(function () {
  ...
  const REGEX = [
    ...
    [/^(long|short) bias · (weak|moderate|strong|very strong)$/,
      (m, s, f) => (s === "long" ? "viés comprador" : "viés vendedor") + " · " + ({ weak: "fraca", moderate: "moderada", strong: "forte", "very strong": "muito forte" })[f]],
    [/^(long|short) (\S+)$/, (m, s, par) => (s === "long" ? "comprado em " : "vendido em ") + par],
    [/^(\S+) relatively stronger than (\S+)$/, "$1 relativamente mais forte que $2"],
    [/^(weak|moderate|strong|very strong) · share of the ceiling the connected dimensions allow \(([\d.]+)\)$/,
      (m, f, t) => ({ weak: "fraca", moderate: "moderada", strong: "forte", "very strong": "muito forte" })[f] + " · fração do teto que as dimensões ligadas permitem (" + t.replace(".", ",") + ")"],
    [/^share of the ceiling the connected dimensions allow \(([\d.]+)\)$/, (m, t) => "fração do teto que as dimensões ligadas permitem (" + t.replace(".", ",") + ")"],
    [/^(\d+) of (\d+)$/, "$1 de $2"],
    [/^dimensions on (\S+) point the same way$/, "dimensões em $1 apontam para o mesmo lado"],
    [/^(\S+) (today|tomorrow|in \d+ days)$/, (m, s, q) => s + " " + (q === "today" ? "hoje" : q === "tomorrow" ? "amanhã" : q.replace(/^in (\d+) days$/, "em $1 dias"))],
    [/^(\S+) decision · (\S+)$/, "decisão do $1 · $2"],
